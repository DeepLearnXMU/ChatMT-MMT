...
def parse_args(args=None):
    ...
    parser.add_argument("--validation_ctx", type=str,
                        help="Path of validation file")
    ...
    parser.add_argument("--train_dialog_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_dialog_ctx_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_dialog_en_label", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_speaker_ctx_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_speaker_en_label", type=str,
                        help="Path of context corpus")

    parser.add_argument("--train_dialog_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_dialog_ctx_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_dialog_de_label", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_speaker_ctx_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--train_speaker_de_label", type=str,
                        help="Path of context corpus")

    parser.add_argument("--dialog_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--dialog_ctx_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--dialog_en_label", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_ctx_en", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_en_label", type=str,
                        help="Path of context corpus")

    parser.add_argument("--dialog_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--dialog_ctx_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--dialog_de_label", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_ctx_de", type=str,
                        help="Path of context corpus")
    parser.add_argument("--speaker_de_label", type=str,
                        help="Path of context corpus")
...
def default_parameters():
    params = tf.contrib.training.HParams(
        ...
        train_steps=200000,
        start_steps=0,
        kl_annealing_steps=5000.0,
        kl_annealing_steps2=5000.0,
        buffer_size=10000,
...
def override_parameters(params, args):
    ...
    params.train_dialog_en = args.train_dialog_en # or params.dialog_src_context
    params.train_dialog_ctx_en = args.train_dialog_ctx_en #or params.dialog_tgt_context
    params.train_dialog_en_label = args.train_dialog_en_label
    params.train_speaker_ctx_en = args.train_speaker_ctx_en #or params.style_src_context
    params.train_speaker_en_label = args.train_speaker_en_label #or params.style_tgt_context

    params.train_dialog_de = args.train_dialog_de # or params.dialog_src_context
    params.train_dialog_ctx_de = args.train_dialog_ctx_de #or params.dialog_tgt_context
    params.train_dialog_de_label = args.train_dialog_de_label
    params.train_speaker_ctx_de = args.train_speaker_ctx_de #or params.style_src_context
    params.train_speaker_de_label = args.train_speaker_de_label #or params.style_tgt_context

    params.dialog_en = args.dialog_en # or params.dialog_src_context
    params.dialog_en_label = args.dialog_en_label
    params.dialog_ctx_en = args.dialog_ctx_en #or params.dialog_tgt_context
    params.speaker_en = args.speaker_en #or params.style_src_context
    params.speaker_en_label = args.speaker_en_label
    params.speaker_ctx_en = args.speaker_ctx_en #or params.style_tgt_context

    params.dialog_de = args.dialog_de # or params.dialog_src_context
    params.dialog_de_label = args.dialog_de_label
    params.dialog_ctx_de = args.dialog_ctx_de #or params.dialog_tgt_context
    params.speaker_de = args.speaker_de #or params.style_src_context
    params.speaker_de_label = args.speaker_de_label
    params.speaker_ctx_de = args.speaker_ctx_de #or params.style_tgt_context
    ...
    params.vocabulary["label"] = vocabulary.process_vocabulary(
        params.vocabulary["label"], params
    )
...
#            features = dataset.get_training_input(params.input, params)
            ...
            dialog_features_en = dataset.get_training_dialogue_input(params.speaker_en, params.speaker_ctx_en, params.speaker_en_label, params.dialog_en, params.dialog_ctx_en, params.dialog_en_label, params) 
            dialog_features_de = dataset.get_training_dialogue_input(params.speaker_de, params.speaker_ctx_de, params.speaker_de_label, params.dialog_de, params.dialog_ctx_de, params.dialog_de_label, params)
        else:
        ...
        for item in s_losses:
            sharded_loss, sp_loss, sp_loss2, coh_loss, coh_loss2 = item[0], item[1], item[2], item[3], item[4]
            sharded_losses.append(sharded_loss)
            sp_losses.append(sp_loss)
            sp_losses2.append(sp_loss2)
            coh_losses.append(coh_loss)
            coh_losses2.append(coh_loss2)

        ce_loss = tf.add_n(sharded_losses) / len(sharded_losses)
        sp_loss = tf.add_n(sp_losses) / len(sp_losses)
        sp_loss2 = tf.add_n(sp_losses2) / len(sp_losses2)
        coh_loss = tf.add_n(coh_losses) / len(coh_losses)
        coh_loss2 = tf.add_n(coh_losses2) / len(coh_losses2)
        ...
        tmp2 = tf.constant(params.kl_annealing_steps, dtype=tf.float32)
        tmp3 = tf.constant(params.kl_annealing_steps2, dtype=tf.float32)
        kl_weights = kl_weights * tf.maximum(1.0 - kstep / tmp2, 0.0)
        kl_weights2 = kl_weights2 * tf.maximum(1.0 - kstep / tmp3, 0.0)

#        total_loss = ce_loss + params.sp_alpha * (kl_weights * sp_loss + (1 - kl_weights) * sp_loss2) + params.dia_alpha * (kl_weights2 * coh_loss + (1 - kl_weights2) * coh_loss2)
        total_loss = ce_loss + kl_weights * (params.sp_alpha * sp_loss + params.dia_alpha * coh_loss) + (1 - kl_weights) * (params.sp_alpha2 * sp_loss2 + params.dia_alpha2 * coh_loss2) 
        print(total_loss, ce_loss, sp_loss, coh_loss)
        # Optimization
        ...
        if params.validation and params.references[0]:
            ...
            files = [params.validation] + [params.dev_dialog_src_context] + [position_file_src_dia] + list(params.references)
