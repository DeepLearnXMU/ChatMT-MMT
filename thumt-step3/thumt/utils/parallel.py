...
def parallel_model(model_fn, features, dialog_features_en, dialog_features_de, devices):
    if len(devices) == 1:
        return [model_fn(features, dialog_features_en, dialog_features_de)]

    features = shard_features(features, devices)
    dialog_features_en = shard_features(dialog_features_en, devices)
    dialog_features_de = shard_features(dialog_features_de, devices)

    outputs = data_parallelism(devices, model_fn, features, dialog_features_en, dialog_features_de)
    return outputs
